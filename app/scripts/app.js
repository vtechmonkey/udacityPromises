...
(function(document) {
  ...
  window.addEventListener('WebComponentsReady', function() {
   ...
   // });
    ...
    // return urls using .map
    /*
  Instructions:
  (1) Use .map to fetch all the planets in parallel.
    (a) Call .map on an array and pass it a function.
    (b) .map will execute the function against each element
         in the array immediately.
   */
  // getJSON('../data/earth-like-results.json')
  // .then(function(response){
  //   var planets = response.results;
  //   addSearchHeader(response.query);
  //   planets.map(function(url){
  //     getJSON(url).then(createPlanetThumb);
  //   });
  getJSON('../data/earth-like-results.json')
  .then(function(response){
